...
flood-wave propagation in a rectangular river channel (which can have
spatially varying width, bed elevation, and roughness).  The model
also includes a fully coupled multi-fraction sediment transport module.

Physical System
---------------
The model represents a straight channel of rectangular cross-section.
Properties can vary along its length when loaded from external geometry files.
All spatial variation is along the streamwise coordinate x.
...
# ── Channel Geometry & Roughness (Fallback Values) ──────────────
# Geometry, roughness, and boundary conditions are now primarily loaded from
# paired CSV files (Flow_Data_##.csv and Channel_Data_##.csv) generated by
# GenBC_1DFlowModel_V2.py. The values below act as fallback values if the
# CSV files fail to load or are not found.
...
LENGTH    = 1000.0           # fallback channel length  L  [m]
WIDTH     = 10.0             # fallback channel width   B  [m]
BED_SLOPE = 0.01             # fallback longitudinal bed slope  S₀ [m/m]
                             # (steep for a natural channel; drives high Froude)
MANNING_N = 0.035            # fallback Manning's roughness coefficient n [s/m^(1/3)]

# ── Numerical Parameters ─────────────────────────────────────────
# DX sets the spatial resolution when fallback geometry is used.
# (If a channel CSV is loaded, its spatial grid is used instead.)
# Finer DX = more accuracy but proportionally more time steps
# (since dt ∝ dx via the CFL condition).
# Rule of thumb: ensure at least 20–50 computational nodes for smooth
# profile resolution (here 1000/20 = 51 nodes).
DX      = 20.0               # fallback spatial grid step Δx  [m]
